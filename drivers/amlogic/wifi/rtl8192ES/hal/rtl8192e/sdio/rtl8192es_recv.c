...
static void rtl8192es_recv_tasklet(void *priv)
{
	...
	do {
		...
		do {
			...
			//rx desc parsing
			rtl8192e_query_rx_desc_status(precvframe,pdesc);
			...
			else
			{
				...
				if(pkt_copy)
				{
					...
					_rtw_memcpy(pkt_copy->data, (pdesc + rx_report_sz + pattrib->shift_sz), skb_len);
				...
				else
				{
					...
					if(precvframe->u.hdr.pkt)
					{
						...
						pkt_clone->data = pdesc + rx_report_sz + pattrib->shift_sz;
				...
				if (pHalData->ReceiveConfig & RCR_APPFCS) {
					...
					if (check_fwstate(mlmepriv, WIFI_MONITOR_STATE) == _FALSE) {
				...
				{
					pphy_status = (pdesc + (rx_report_sz - pattrib->drvinfo_sz));
