...
*/	
	...
	ODM_SetTxAntByTxInfo(&GET_HAL_DATA(padapter)->odmpriv, ptxdesc, pxmitframe->attrib.mac_id);
#endif
...
//todo: static 
s32 rtl8192es_dequeue_writeport(PADAPTER padapter)
{
...
s32 rtl8192es_xmit_buf_handler(PADAPTER padapter)
{
	...
	do {
		queue_empty = rtl8192es_dequeue_writeport(padapter);
//	dump secondary adapter xmitbuf 
#ifdef CONFIG_CONCURRENT_MODE
		if(rtw_buddy_adapter_up(padapter))
			queue_empty &= rtl8192es_dequeue_writeport(padapter->pbuddy_adapter);
#endif
