...
void vivid_thread_vid_out_tick(struct vivid_dev *dev)
{
	...
	if (vid_out_buf) {
		...
		vivid_get_timestamp(&vid_out_buf->vb);
		vivid_wrap_time_offset(&vid_out_buf->vb, dev->time_wrap_offset);
	...
	if (vbi_out_buf) {
		...
		vivid_get_timestamp(&vbi_out_buf->vb);
		vivid_wrap_time_offset(&vbi_out_buf->vb, dev->time_wrap_offset);
