...
#define TEGRA_UART_RX_DMA_BUFFER_SIZE		(2 * 4096)
...
struct tegra_uart_port {
	...
	dma_addr_t				rx_ring_dma_buf_phys;
...
static inline void tegra_uart_write(struct tegra_uart_port *tup,
	unsigned int val, unsigned long reg)
{
				      ...
				      unsigned int count)
{
	...
	dma_sync_single_for_cpu(tup->uport.dev, tup->rx_ring_dma_buf_phys,
				count, DMA_FROM_DEVICE);
	copied = tty_insert_flip_string(tty, rx_buf, count);
	if (copied != count) {
		dev_err(tup->uport.dev, "RxData DMA copy to tty layer failed\n");
		ret = -ENOSPC;
	}
	dma_sync_single_for_device(tup->uport.dev, tup->rx_ring_dma_buf_phys,
				count, DMA_TO_DEVICE);
	return ret;
}
		...
		unsigned int count)
{
	...
	if (ring->head + count > tup->rx_total_bytes)
		ring->head = ring->head + count - tup->rx_total_bytes;
	else
		ring->head += count;

	if (ring->head < ring->tail) {
		count = tup->rx_total_bytes - ring->tail;
		rx_buf = ring->buf + ring->tail;
		tup->rx_ring_dma_buf_phys = tup->rx_dma_buf_phys + ring->tail;
		ret = tegra_uart_copy_rx_to_tty(tup, port, rx_buf, count);
		ring->tail = 0;
		if (ret)
			return ret;
	}
...
static void tegra_uart_rx_dma_complete(void *args)
{
	...
	tup->curr_xfer_count = state.total_bytes_transferred;
	count = tup->curr_xfer_count - tup->prev_xfer_count;
	tup->prev_xfer_count = tup->curr_xfer_count;

	if (!count)
		goto exit;

	tegra_uart_rx_buffer_push(tup, count);
exit:
	spin_unlock_irqrestore(&u->lock, flags);
}
...
static int tegra_uart_start_rx_dma(struct tegra_uart_port *tup)
{
	...
	tup->rx_period_len = count/2;
	tup->rx_dma_desc = dmaengine_prep_dma_cyclic(tup->rx_dma_chan,
			tup->rx_dma_buf_phys, count, tup->rx_period_len,
...
static void tegra_uart_stop_rx(struct uart_port *u)
{
	...
	if (tup->rx_dma_chan) {
		...
		dmaengine_tx_status(tup->rx_dma_chan, tup->rx_cookie,
					   &state);
		tup->curr_xfer_count = state.total_bytes_transferred;
		count = tup->curr_xfer_count - tup->prev_xfer_count;
		tup->prev_xfer_count = tup->curr_xfer_count;
...
static int tegra_uart_hw_init(struct tegra_uart_port *tup)
{
	 ...
	 * then the EORD.
	 */
	tup->ier_shadow = UART_IER_RLSI | TEGRA_UART_IER_EORD | UART_IER_RTOIE;
			...
			bool dma_to_memory)
{
	...
	if (dma_to_memory) {
		...
		tup->rx_ring.buf = dma_buf;
		tup->rx_dma_buf_phys = dma_phys;
		tup->rx_ring_dma_buf_phys = dma_phys;
	} else {
...
static int tegra_uart_startup(struct uart_port *u)
{
	...
	tup->rx_ring.head = tup->rx_ring.tail = 0;
	tup->prev_xfer_count = tup->curr_xfer_count = 0;
	return 0;
	...
	struct tegra_uart_port *tup)
{
	...
	if (n_entries > 0) {
		...
		tup->baud_tolerance = devm_kzalloc(&pdev->dev,
				(tup->n_adjustable_baud_rates) *
				sizeof(*tup->baud_tolerance), GFP_KERNEL);
		if (!tup->baud_tolerance)
			return -ENOMEM;
		for (count = 0, index = 0; count < n_entries;
				count += 3, index++) {
...
static const struct of_device_id tegra_uart_of_match[] = {
	{
		.compatible     = "nvidia,tegra186-hsuart-nohw",
